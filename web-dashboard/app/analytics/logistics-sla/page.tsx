import AutoRefresh from "@/components/AutoRefresh";
import { supabase } from "@/lib/supabase-client";
import KPICard from "@/components/analytics/KPICard";
import FulfillmentTimelineChart from "@/components/analytics/logistics-sla/FulfillmentTimelineChart";
import DeliveryMapChart from "@/components/analytics/logistics-sla/DeliveryMapChart";
import TopNodesChart from "@/components/analytics/logistics-sla/TopNodesChart";

export const dynamic = "force-dynamic";

type LogisticsTotals = {
  total_packages: number;
  avg_processing_days: number;
  late_processing: number;
  total_shipped: number;
};

type TopNodeRow = {
  name: string;
  value: number;
};

type MapRow = {
  city: string;
  lat: number;
  lon: number;
  packages: number;
};

type TimelineRow = {
  date: string;
  total_packages: number;
  shipped_packages: number;
};

type LogisticsDashboardData = {
  totals: LogisticsTotals;
  top_nodes: TopNodeRow[];
  map_data: MapRow[];
  timeline: TimelineRow[];
};

export default async function LogisticsSLADashboard() {
  const { data, error } = await supabase.rpc(
    "get_logistics_sla_dashboard_data"
  );

  if (error) {
    return (
      <div className="dashboard-panel border-danger/30 p-6 text-danger">
        Error: {error.message}
      </div>
    );
  }

  if (!data) {
    return (
      <div className="dashboard-panel flex min-h-96 items-center justify-center border-dashed p-6 text-muted">
        No logistics data available.
      </div>
    );
  }

  const dashboardData = data as LogisticsDashboardData;

  const totals = dashboardData.totals;

  const totalPackages = Number(totals.total_packages ?? 0);
  const avgProcessingDays = Number(
    totals.avg_processing_days ?? 0
  );
  const lateProcessing = Number(totals.late_processing ?? 0);
  const totalShipped = Number(totals.total_shipped ?? 0);

  const topNodesData = (dashboardData.top_nodes ?? []).map(
    (item) => ({
      name: String(item.name),
      value: Number(item.value ?? 0),
    })
  );

  const mapData = (dashboardData.map_data ?? []).map(
    (item) => ({
      city: String(item.city),
      lat: Number(item.lat ?? 0),
      lon: Number(item.lon ?? 0),
      packages: Number(item.packages ?? 0),
    })
  );

  const timelineData = (dashboardData.timeline ?? [])
    .map((item) => ({
      date: String(item.date),
      totalPackages: Number(item.total_packages ?? 0),
      shippedPackages: Number(item.shipped_packages ?? 0),
    }))
    .sort((a, b) => a.date.localeCompare(b.date));

  return (
    <main className="flex min-h-screen flex-col gap-7 rounded-lg border border-border bg-surface p-5 md:p-8">
      <AutoRefresh intervalMs={60000} />

      <header className="flex flex-col gap-3 border-b border-border pb-6 md:flex-row md:items-end md:justify-between">
        <div>

          <h1 className="mt-2 text-3xl font-semibold tracking-tight text-foreground md:text-4xl">
            Logistics-SLA
          </h1>

          <p className="mt-2 max-w-2xl text-sm leading-6 text-muted">
            Real-time visibility into fulfillment, shipping, and distribution performance.
          </p>
        </div>

        <div className="font-mono text-[11px] uppercase tracking-widest text-muted">
          Network operations · live
        </div>
      </header>

      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard
          title="Packages in flow"
          value={(totalPackages / 1000).toFixed(1) + "K"}
          colorTheme="blue"
          subtitle="Unique package records"
        />

        <KPICard
          title="Avg processing"
          value={avgProcessingDays.toFixed(2)}
          colorTheme="emerald"
          subtitle="Days per package"
        />

        <KPICard
          title="Late processing"
          value={(lateProcessing / 1000).toFixed(2) + "K"}
          colorTheme="rose"
          subtitle="SLA exceptions"
        />

        <KPICard
          title="Total shipped"
          value={(totalShipped / 1000).toFixed(1) + "K"}
          colorTheme="emerald"
          subtitle="Outbound throughput"
        />
      </section>

      <section className="grid gap-5 lg:grid-cols-[minmax(0,1.55fr)_minmax(280px,.75fr)]">
        <div className="dashboard-panel min-h-[450px] border-success/25 p-5">
          <div className="mb-3 flex items-start justify-between">
            <div>
              <p className="eyebrow text-success">
                Global Distribution view
              </p>

              <h2 className="mt-1 text-xl font-semibold text-foreground">
                Distribution network
              </h2>
            </div>

            <span className="status-dot">
              ● Nodes online
            </span>
          </div>

          <DeliveryMapChart data={mapData} />
        </div>

        <div className="dashboard-panel min-h-[450px] p-5">

          <h2 className="mt-1 text-xl font-semibold text-foreground">
            Top distribution nodes
          </h2>

          <div className="mt-5">
            <TopNodesChart data={topNodesData} />
          </div>
        </div>
      </section>

      <section className="dashboard-panel p-5">
        <div className="mb-4 flex flex-col gap-1 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="eyebrow text-muted">
              Operational Flow
            </p>

            <h2 className="mt-1 text-xl font-semibold text-foreground">
              Order Processing & Delivery Trends
            </h2>
          </div>

          <p className="text-xs text-muted">
            Created vs shipped by month
          </p>
        </div>

        <FulfillmentTimelineChart data={timelineData} />
      </section>
    </main>
  );
}