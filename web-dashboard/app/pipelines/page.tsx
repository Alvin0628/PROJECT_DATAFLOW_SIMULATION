...
export default async function PipelinesPage() {
  ...
  if (body.error) {
    return (
      <div className="dashboard-panel border-danger/30 bg-danger-soft p-5 text-sm text-danger">
        Unable to reach Airflow: {body.error}
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-8 pb-10">
      ...
      <header className="border-b border-border pb-7">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-6 text-muted">
          Observe workflow health, execution posture, and scheduling context across the Airflow runtime.
        </p>
      </header>

      <section className="grid gap-4 md:grid-cols-4">
        <div className="dashboard-panel bg-primary/10 p-5">
          <p className="dashboard-eyebrow">Workflow health</p>
          <p className="mt-3 text-3xl font-semibold text-success">Operational</p>
        </div>
        <div className="dashboard-panel p-5">
          <p className="dashboard-eyebrow">Active DAGs</p>
          <p className="mt-3 text-3xl font-semibold">{active}</p>
          <p className="text-sm text-muted">of {dags.length} registered</p>
        </div>
        <div className="dashboard-panel p-5">
          <p className="dashboard-eyebrow">Paused</p>
          <p className="mt-3 text-3xl font-semibold">{dags.length - active}</p>
          <p className="text-sm text-muted">manual intervention</p>
        </div>
        <div className="dashboard-panel p-5">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
        <div className="dashboard-panel p-5 md:p-6">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-full bg-success-soft px-3 py-1 font-mono text-[10px] text-success">
              {active} RUNNING
            </span>
          </div>
          <div className="mt-8 flex flex-col gap-3">
            {["Ingest", "Transform", "Train", "Infer", "Serve"].map((step, index) => (
              <div key={step} className="flex items-center gap-4">
                ...
                <div className="w-28 font-mono text-xs text-muted">{step}</div>
        ...
        <div className="dashboard-panel p-5 md:p-6">
          ...
          <div className="mt-6 flex flex-col gap-4">
            {dags.slice(0, 4).map((dag) => (
              <div key={dag.dag_id} className="border-b border-border pb-4 last:border-0">
                <p className="truncate font-mono text-xs font-semibold">{dag.dag_id}</p>
          ...
          <table className="dashboard-table w-full min-w-[720px] text-left text-sm">
            ...
            <tbody>
              {dags.length === 0 ? (
                ...
                dags.map((dag) => (
                  <tr key={dag.dag_id}>
                    <td className="px-5 py-4 font-mono text-xs font-semibold">{dag.dag_id}</td>
                    ...
                    <td className="px-5 py-4 text-muted">{dag.owners.join(", ")}</td>
