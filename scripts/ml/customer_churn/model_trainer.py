...
from scripts.ml.customer_churn.data_loader import load_churn_data
from scripts.ml.customer_churn.preprocessor import clean_structural_data, get_sklearn_preprocessor
...
def diagnose_feature_signal(X: pd.DataFrame, y: pd.Series, categorical_features: list, numeric_features: list):
    ...
    if numeric_features:
        ...
        mi_numeric = mutual_info_classif(X_num, y, random_state=SEED)
...
def train_model():
    ...
    dynamic_ratio = count_class_0 / count_class_1 if count_class_1 > 0 else 1.0

    print(f"Rasio Dasar scale_pos_weight: {dynamic_ratio:.4f}")
    ...
    # =========================================================================
    output_dir_model = "/opt/airflow/models/customer_churn"
    output_dir_test_set = os.path.join(output_dir_model, "test_set")
    
    os.makedirs(output_dir_model, exist_ok=True)
    os.makedirs(output_dir_test_set, exist_ok=True)
    
    existing_models = glob.glob(os.path.join(output_dir_model, "customer_churn_model_v*.joblib"))
    versions = []
    ...
    joblib.dump(final_pipeline, os.path.join(output_dir_model, f"customer_churn_model_{v_tag}.joblib"))
